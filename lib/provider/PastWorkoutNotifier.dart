...
class PastWorkoutNotifier extends ChangeNotifier {
  ...
  UnmodifiableListView<PastWorkout> get pastWorkouts =>
      UnmodifiableListView(_pastWorkoutList);
  ...
  Future<void> updatePastWorkout(PastWorkout pastWorkout,
      {String name, DateTime date, String notes}) async {
    if (name != null && name.length > 0) pastWorkout.workout.name = name;
    if (date != null) pastWorkout.dateTime = date;
    if (notes != null) pastWorkout.notes = notes;
    notifyListeners();
